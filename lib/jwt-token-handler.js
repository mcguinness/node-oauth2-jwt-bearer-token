      ...
      .then(key => {
        Jwt.verify(
          ...
          function(err, claims) {
            ...
            // If audience is array, then check if contains options audience
            // if is not an array, so it's a string, then just compare it
            if (Array.isArray(claims.aud) && !claims.aud.includes(options.audience) ||
              (!Array.isArray(claims.aud) && claims.aud !== options.audience)) {
